%% A demo of running LCA on natural video with monophasic and biphasic LGN
...
lgn.tau = 1;
tmp = []; tmp2 = []; tau = lgn.tau;
for t = 0 : 10
    tmp(end+1) = (t^6*exp(-t/(tau/7)^(1/2)))/(720*(tau/7)^(7/2));
    tmp2(end+1) = (t^5*exp(-t/(tau/7)^(1/2)))/(120*(tau/7)^(7/2)) - ...
        (2401*t^6*exp(-t/(tau/7)^(1/2)))/(720*tau^4);
end

lgn.n_c = 1.8; % Biphasic multiplicative factor
lgn.on_temp_filt_coef = lgn.n_c * tmp2(end:-1:1)';
lgn.off_temp_filt_coef = lgn.on_temp_filt_coef; % biphasic
lgn.on_temp_filt_coef2 = tmp(end:-1:1)';
lgn.off_temp_filt_coef2 = lgn.on_temp_filt_coef2; % monophasic
...
lca.a_norm = 1;
lca.A = normalize_matrix(randn(4*L, M), 'L2 norm', lca.a_norm); 
% Connections between input and output layer

X = zeros(4*L, lca.batch_size); 
...
for i_epoch = 1 : lca.n_epoch
    ...
    while i_frame <= num_frames
        ...
        for i_batch=1:lca.batch_size 
            ...
            X_data_ON2(:,i_batch) = X_spatiotemporal(:,:,i_batch) * ...
                lgn.on_temp_filt_coef2(filter_index); 
            X_data_OFF2(:,i_batch) = X_spatiotemporal(:,:,i_batch) * ...
                lgn.off_temp_filt_coef2(filter_index); 
        end
        
        % ON and OFF LGN input
        X_ON = max( X_data_ON, 0 );
        X_OFF = - min( X_data_OFF, 0 );
        X_ON2 = max( X_data_ON2, 0 );
        X_OFF2 = - min( X_data_OFF2, 0 );
        X( 1:L, : ) = X_ON;
        X( L+1:2*L, : ) = X_OFF;
        X( 2*L+1:3*L, : ) = X_ON2;
        X( 3*L+1:4*L, : ) = X_OFF2;
        ...
        if (mod(i_frame,display_every) == 0)
            ...
            figure(11); % Display the connections from ON and OFF LGN cells 
            % to simple cells
            subplot 131; DisplayA( 'ONOFF', lca.A(2*L+1:4*L,:), ...
                resize_factor); title('RFs: A_{ON}-A_{OFF}');
            subplot 132; DisplayA( 'ON', lca.A(2*L+1:4*L,:), ...
                resize_factor ); title('A_{ON}');
            subplot 133; DisplayA( 'OFF', lca.A(2*L+1:4*L,:), ...
                resize_factor ); title('A_{OFF}');
            colormap(scm(256));
            
            figure(2);
            subplot(211);stem(S);title(['S: firing rates of ' ...
